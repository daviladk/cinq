...
impl Qora {
    pub fn new() -> Self {
        ...
        // Message templates
        templates.insert("message_sent", vec![
        ...
        templates.insert("message_failed", vec![
        ...
        // Usage templates
        templates.insert("usage_report", vec![
        ...
        // Upload templates
        templates.insert("upload_starting", vec![
            "Uploading {file_name}...",
            "📤 Sending {file_name} to storage...",
        ]);
        
        templates.insert("upload_complete", vec![
        ...
        // Download templates
        templates.insert("download_complete", vec![
        ...
        templates.insert("help_message", vec![
            "To send a message: 'send alice hello there' or 'message bob how are you?'",
        ]);
        
        templates.insert("help_call", vec![
            "To call: 'call alice' or 'video call bob'. End with 'hang up'.",
        ]);
        
        templates.insert("help_balance", vec![
            "Check your Qi: 'balance' or 'how much qi do i have?'",
        ]);
        
        templates.insert("help_storage", vec![
            "Upload: 'upload /path/to/file'. Download: 'download [file-id]'.",
        ]);
        
        // Unknown/fallback templates
        templates.insert("unknown", vec![
    ...
    fn generate_initial_response(&self, intent: &Intent) -> String {
        match intent {
            ...
            Intent::Unknown { .. } => {
                self.render("unknown", &ResponseContext::default())
            }
        }
    }
    ...
    pub fn generate_completion_response(
        ...
        match intent {
            ...
            Intent::CheckUsage => {
                self.render("usage_report", ctx)
            }
            Intent::UploadFile { .. } => {
                self.render("upload_complete", ctx)
            }
            Intent::DownloadFile { .. } => {
                self.render("download_complete", ctx)
            }
            Intent::AddContact { .. } => {
                self.render("contact_added", ctx)
            }
            Intent::ListContacts => {
                self.render("contacts_list", ctx)
            }
            Intent::SearchMessages { .. } => {
                if ctx.count > 0 {
    ...
    fn render(&self, key: &str, ctx: &ResponseContext) -> String {
        let templates = self.templates.get(key).unwrap_or_else(|| {
            self.templates.get("unknown").unwrap()
        });
    ...
    pub fn intent_to_action_type(&self, intent: &Intent) -> Option<ActionType> {
        match intent {
            ...
            Intent::UploadFile { .. } | Intent::DownloadFile { .. } => Some(ActionType::FileTransfer),

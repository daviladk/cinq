...
impl IntentParser {
    pub fn new() -> Self {
        ...
        // Message patterns
        patterns.insert("message", vec![
            "send", "message", "msg", "tell", "text", "dm", "write to",
        ]);
        
        // Call patterns
        patterns.insert("call", vec![
            "call", "phone", "ring", "dial",
        ]);
        
        // Video call patterns
        patterns.insert("video", vec![
            "video", "facetime", "video call", "video chat",
        ]);
        
        // End call patterns
        patterns.insert("hangup", vec![
            "hang up", "end call", "disconnect", "stop call",
        ]);
        
        // Upload patterns
        patterns.insert("upload", vec![
            "upload", "store", "save", "backup", "sync",
        ]);
        
        // Download patterns
        patterns.insert("download", vec![
            "download", "get file", "fetch", "retrieve",
        ]);
        
        // Balance patterns
        patterns.insert("balance", vec![
            "balance", "how much", "qi left", "funds", "wallet",
        ]);
        
        // Usage patterns
        patterns.insert("usage", vec![
            "usage", "spent", "consumed", "session", "cost so far",
        ]);
        
        // Estimate patterns
        patterns.insert("estimate", vec![
            "estimate", "how much would", "cost of", "price",
        ]);
        
        // Contact patterns
        patterns.insert("contact", vec![
            "add contact", "new contact", "save contact",
        ]);
        
        // List contacts patterns
        patterns.insert("list_contacts", vec![
            "contacts", "list contacts", "show contacts", "who",
        ]);
        
        // Search patterns
        patterns.insert("search", vec![
            "search", "find", "look for", "where did",
        ]);
        
        // Help patterns
        patterns.insert("help", vec![
            "help", "how do i", "what can", "commands", "?",
        ]);
        
        // Settings patterns
        patterns.insert("settings", vec![
            "set", "change", "configure", "preference", "setting",
        ]);
    ...
    pub fn parse(&self, input: &str) -> Intent {
        ...
        if self.matches(&lower, "video") {
            if let Some(recipient) = self.extract_recipient(&lower, &["video call", "video chat", "video"]) {

...
impl StorageWorker {
    ...
    pub async fn search_messages(&self, query: &str) -> WorkerResult {
        ...
        WorkerResult::ok(format!("Found {} messages", count)).with_data(serde_json::json!({
            "count": count,
    ...
    pub async fn store_file_local(&self, name: &str, data: &[u8], mime_type: &str) -> WorkerResult {
        let file = StoredFile {
            id: uuid::Uuid::new_v4().to_string(),
            name: name.to_string(),
    ...
    pub async fn upload_to_cloud(&self, file_id: &str) -> WorkerResult {
        ...
        if let Some(file) = files.iter_mut().find(|f| f.id == file_id) {
            if file.location == FileLocation::CloudBackup
                || file.location == FileLocation::CloudOnly
            {
...
mod tests {
    ...
    async fn test_file_storage() {
        ...
        let result = worker
            .store_file_local("test.txt", data, "text/plain")
            .await;

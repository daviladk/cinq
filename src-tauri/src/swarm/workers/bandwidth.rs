...
impl BandwidthWorker {
    ...
    pub async fn start_call(&self, peer_id: &str) -> WorkerResult {
        let stream_id = self.start_stream(peer_id.to_string(), StreamType::VoiceCall).await;
    ...
    pub async fn start_video_call(&self, peer_id: &str) -> WorkerResult {
        let stream_id = self.start_stream(peer_id.to_string(), StreamType::VideoCall).await;
...
mod tests {
    ...
    async fn test_call_flow() {
        ...
        let stream_id = start_result.data.unwrap()["stream_id"].as_str().unwrap().to_string();

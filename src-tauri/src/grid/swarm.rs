...
impl SwarmCoordinator {
    ...
    pub fn upgrade_tier(&mut self, tier: AgentTier) {
        ...
        if tier >= AgentTier::Light {
            self.local_agent
                .capabilities
                .push(Capability::LocalInference);
        }
    }
    ...
    pub async fn handle_message(&mut self, msg: SwarmMessage) -> Option<SwarmMessage> {
        match msg {
            ...
            SwarmMessage::TaskProgress {
                task_id,
                progress,
                message: _,
            } => {
                if let Some(task) = self.tasks.get_mut(&task_id) {
                    if let TaskState::Assigned { agent } | TaskState::InProgress { agent, .. } =
                        &task.state
                    {
                        task.state = TaskState::InProgress {
                            agent: agent.clone(),
            ...
            SwarmMessage::TaskComplete { task_id, result } => {
                if let Some(task) = self.tasks.get_mut(&task_id) {
                    if let TaskState::InProgress { agent, .. } | TaskState::Assigned { agent } =
                        &task.state
                    {
                        task.state = TaskState::Completed {
                            agent: agent.clone(),
            ...
            SwarmMessage::GetTaskStatus { task_id } => self
                .tasks
                .get(&task_id)
                .cloned()
                .map(SwarmMessage::TaskStatus),
            SwarmMessage::GetPendingTasks => {
                let pending: Vec<SwarmTask> = self
                    .pending_queue
                    .iter()
...
mod tests {
    ...
    fn test_task_lifecycle() {
        ...
        coord
            .complete_task(&task_id, "Result!".to_string(), 10)
            .unwrap();
